...
- [x] **DONE, with a corrected finding — see `plans/gscript-real-source-notes.md`'s arrays-step-4 addendum for the full account.** The original expectation here (failure LINE stays the same, only the CATEGORY changes from parse error to the pre-existing entity-deferred compile rejection) turned out to be based on an ISOLATED, synthetic reproduction, not the real whole file — real result is BETTER: `CompileGscSource` parses the WHOLE file as one `Program` before any compilation begins, so once `[]`/`[key]` parse (steps 1-3), parsing sails straight past every `level.field[key]` use in between and only stops at the NEXT genuinely unparseable construct. For all three real levels that turns out to be `thread` (no parser grammar exists for it at all): killhouse moves from line 204 to line 221, bog_a from 71 to 89, cargoship from 10 to 158 — all landing on the exact same NEW gap (`thread`), which is real, verified, honest progress, just further than predicted and for a different reason (whole-file parse gating, not a per-construct category change). hunted is correctly unchanged (line 5, `#`-directive, no array construct involved). The isolated, synthetic `level.field[key]` reproduction DOES still show the originally-expected category change in the addendum's own test — that mechanism is real and correct, it's just not what the WHOLE FILE'S first error looks like once threading is now the earlier blocker. Isolated real-corpus snippets (`C4_models[i] hide()`, `aa[i] hide()/notsolid()`, `targets[i] thread moveTargetDummy(...)`, and the real `tooslow_dialog = []; tooslow_dialog[0..3] = ...;` block, verbatim including its own real init line at 909) all confirmed correct: the array construct itself is fine everywhere, rejected (where rejected) only by a separately-tracked, correctly-named, unrelated gap (entity method calls or threading), never by a confusing array-specific error.
...
- [ ] Run the full pipeline on real device (`New Game` -> `devmap killhouse`, same navigation established across this entire session) and confirm: no crash, killhouse's failure point matches Step 4's CORRECTED host finding (see `gscript-real-source-notes.md`'s arrays-step-4 addendum) — the real failure LINE moves from 204 to **221** (a `thread` parse error, since `CompileGscSource`/`CompileGscZoneEntryPoint` both parse the whole file before compiling, and parsing now sails past every `level.field[key]` use between the old and new line), NOT "line 204, different category" as originally assumed pre-Step-4. Confirm this real device number matches the host finding exactly; if it differs, that's a real discrepancy between the host pipeline and the real device zone-loader path worth investigating, not something to paper over.
...
- Device pass confirms no crash, no regressions, and the CORRECTED expected result: killhouse's real failure point at line 221 (a `thread` parse error), matching Step 4's host finding exactly — not the originally-predicted "unchanged line 204, different category" (see the arrays-step-4 addendum for why that prediction was superseded by a better real result).
